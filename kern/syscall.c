...
sys_exofork(void)
{
	...
	struct Env *newenv;
	int re = env_alloc(&newenv, curenv->env_id);
	if(re < 0)
		return re;
...
sys_env_set_status(envid_t envid, int status)
{
	...
	if(status != ENV_RUNNABLE && status != ENV_NOT_RUNNABLE)
		return -E_INVAL; // not a valid status for an env

	struct Env *env;
	int re = envid2env(envid, &env, 1); // set the 3rd param to 1
	if(re < 0)
		return -E_BAD_ENV;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	re = page_insert(env->env_pgdir, p, va, perm);
	if(re < 0)
		return -E_NO_MEM;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((uint32_t)srcva >= UTOP || (uint32_t)srcva % PGSIZE)
		return -E_INVAL;
	else if((uint32_t)dstva >= UTOP || (uint32_t)dstva % PGSIZE)
		return -E_INVAL;
	...
	re = page_insert(dstenv->env_pgdir, p, dstva, perm);
	if(re < 0)
		return -E_NO_MEM;
